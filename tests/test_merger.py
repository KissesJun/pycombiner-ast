...
import tempfile
import shutil
from ..combiner.merger import merge_files, read_file
...
'''
    ...
    def test_read_file(self):
        """Test reading file content with different encodings"""
        # Test UTF-8 file
        ...
        special_content = "Special chars: é, ñ, ü"
        special_file = self.source_dir / "special.py"
        special_file.write_text(special_content, encoding='utf-8')
        content = read_file(special_file)
        self.assertEqual(content, special_content)
        
    def test_merge_files(self):
        """Test merging files into a single file"""
        ...
        merge_files(python_files, {}, self.source_dir, output_file, entry_file)
    ...
    def test_merge_files_with_circular_dependency(self):
        """Test merging files with circular dependencies"""
...
def func_b():
    return func_a()
'''
        (self.source_dir / "a.py").write_text(a_content)
        (self.source_dir / "b.py").write_text(b_content)
        ...
        # Merge files
        entry_file = self.source_dir / "a.py"
        merge_files(python_files, {}, self.source_dir, output_file, entry_file)
        ...
        self.assertIn("def func_a()", content)
        self.assertIn("def func_b()", content)
